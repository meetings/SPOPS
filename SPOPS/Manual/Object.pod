...
creates a subroutine in the namespace of your class to handle
successive calls.
...
field rather than a string. (Wisenheimers who use an arrayref with one
element may be shocked that SPOPS finds this attempt to trick it and
sets the value to the single element.)
