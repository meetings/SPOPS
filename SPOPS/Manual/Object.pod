...
but what happens when you want to clear out a value?
...
Instead call the '_clear' method associated with a fieldname:

 $user->_m_name_clear;

This explicitly sets the value to undef.

The system accomplishes this using AUTOLOAD, and after the first call
it automatically creates a subroutine in the namespace of your class
to handle successive calls. If you require you can modify how these
two methods get created by overriding
C<_internal_create_field_methods()>. This takes three arguments: the
argument being modified, the class to install the routines into, and
the fieldname used to create the methods. Don't implement this unless
you know what you're doing -- check out the implementation in
